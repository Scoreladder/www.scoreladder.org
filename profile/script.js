...
async function load() {
  try {
    ...
    if (sessionId) {
      meUrl = `${Auth_API}/me?session=${encodeURIComponent(sessionId)}`;
    }
    ...
    try {
      const ranks = await import("../ranks.js");

      if (typeof ranks.getMineralRank === "function") {
        getMineralRank = ranks.getMineralRank;
      } else {
        console.error("ranks.js does not export getMineralRank");
      }
    } catch (rankError) {
      console.error("Failed to load ranks.js:", rankError);
    }

    renderProfile(user, getMineralRank);
  } catch (error) {
...
function renderProfile(user, getMineralRank) {
  ...
  const rwEloHidden = Boolean(stats.rw_elo_hidden);

  const mathEloHidden = Boolean(stats.math_elo_hidden);

  const rwElo = rwEloHidden ? null : Number(stats.rw_elo);

  const mathElo = mathEloHidden ? null : Number(stats.math_elo);

  // ==========================================================
  // MINERAL RANKS
  // ==========================================================

  const rwRank = !rwEloHidden && getMineralRank ? getMineralRank(rwElo) : null;

  const mathRank =
    !mathEloHidden && getMineralRank ? getMineralRank(mathElo) : null;
  ...
  document.getElementById("id").innerText = user.username
    ? `@${user.username}`
    : user.id;
  ...
  banner.style.background = profile.banner
  ...
  const rwAnswered = Number(stats.rw_questions_answered) || 0;

  const rwCorrect = Number(stats.rw_questions_correct) || 0;

  const mathAnswered = Number(stats.math_questions_answered) || 0;

  const mathCorrect = Number(stats.math_questions_correct) || 0;

  const rwAccuracy =
    rwAnswered > 0 ? Math.round((rwCorrect / rwAnswered) * 100) : 0;

  const mathAccuracy =
    mathAnswered > 0 ? Math.round((mathCorrect / mathAnswered) * 100) : 0;
...
function renderQuestionTypeStats(questionTypeStats) {
  const existing = document.getElementById("questionTypeStats");
  ...
  title.innerText = "Reading & Writing Performance";
  ...
  for (const stat of questionTypeStats) {
    const answered = Number(stat.questions_answered) || 0;

    const correct = Number(stat.questions_correct) || 0;

    const accuracy = answered > 0 ? Math.round((correct / answered) * 100) : 0;

    const row = document.createElement("div");

    row.className = "question-type-stat";

    const name = document.createElement("span");

    name.className = "question-type-name";

    name.innerText = names[stat.question_type] || stat.question_type;

    const accuracyElement = document.createElement("span");

    accuracyElement.className = "question-type-accuracy";

    accuracyElement.innerText = `${accuracy}%`;

    const count = document.createElement("span");

    count.className = "question-type-count";

    count.innerText = `${correct}/${answered}`;
  ...
  if (questionTypeStats.length === 0) {
    ...
    empty.innerText = "No Reading & Writing data yet.";
