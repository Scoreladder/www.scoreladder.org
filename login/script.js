...
function initializeScoreInputs() {
  populateScoreDropdown("rwScoreInput");

  populateScoreDropdown("mathScoreInput");
}
...
async function loadUser() {
  try {
    ...
    if (urlSession) {
      ...
      try {
        session = sessionStorage.getItem(LOCAL_SESSION_KEY);
      } catch {
        session = null;
      }
    }
    ...
    if (session) {
      meUrl.searchParams.set("session", session);
    }
    ...
    */
    const rwElo = user.stats?.rw_elo;

    const mathElo = user.stats?.math_elo;

    if (isLoginPage && (rwElo == null || mathElo == null)) {
      showScoreSetup();
    ...
    if (avatar) {
      if (user.id?.startsWith("discord_")) {
        ...
        if (user.avatar) {
          avatar.src = /^https?:\/\//i.test(user.avatar)
            ? user.avatar
    ...
    if (isLoginPage) {
      const profileUrl = new URL(`${window.location.origin}/profile/`);
...
async function logout() {
  try {
    ...
    try {
      ...
      console.error("Could not clear local session:", error);
    }
...
async function setUsername() {
  ...
  try {
    ...
    console.error("Could not read local session:", error);
  }
  ...
  try {
    ...
    if (!res.ok) {
      error.textContent = data.error || "Could not set username.";
...
async function submitInitialScores() {
  ...
  const rwInput = document.getElementById("rwScoreInput");
  ...
  const rwScore = Number(rwInput?.value);

  const mathScore = Number(mathInput?.value);

  if (!Number.isInteger(rwScore) || rwScore < 200 || rwScore > 800) {
    error.textContent = "Please select your Reading & Writing score.";

    return;
  }

  if (!Number.isInteger(mathScore) || mathScore < 200 || mathScore > 800) {
    error.textContent = "Please select your Math score.";
  ...
  try {
    ...
    if (!res.ok) {
      error.textContent = data.error || "Could not save your starting rating.";
    ...
    */
    ...
    console.error("Failed to submit initial scores:", e);
...
window.submitInitialScores = submitInitialScores;
