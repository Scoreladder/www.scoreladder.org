...
export function handleGameStart(data, isResume = false) {
  /*
   * A match that has already received its final result
   * must never be started again.
   */
  if (state.matchFinished) {
    console.warn("Ignoring game_start for a match that is already finished.", {
  ...
  if (!questions) {
    ...
    setStatus("The server sent an invalid question set.");
   ...
   */
  const rawStartTime = Array.isArray(data)
  ...
  if (!Number.isFinite(startTime) || startTime <= 0) {
    console.error("Invalid match start time:", rawStartTime);

    setStatus("Unable to start match timer.");
  ...
  const savedAnswers = Array.isArray(state.selectedAnswers)
    ? [...state.selectedAnswers]
    : [];

  state.questions = questions.slice(0, TOTAL_QUESTIONS).map((question) => {
  ...
  if (isResume && savedAnswers.length === state.questions.length) {
    state.selectedAnswers = savedAnswers;
  } else {
    state.selectedAnswers = new Array(TOTAL_QUESTIONS).fill(-1);
  }
  ...
  state.matchStartedAt = startTime;

  state.challengeDeadline = startTime + MATCH_DURATION_MS;
  ...
  state.timeRemaining = Math.max(
    0,
    Math.ceil((state.challengeDeadline - Date.now()) / 1000),
  );
  ...
  if (elements.submitButton) {
    ...
    elements.submitButton.disabled =
      state.challengeSubmitted || state.submissionInProgress;
  }
...
export function startResumedGame(questions, startTime) {
  startGame(questions, startTime, true);
}
...
export function startMatchTimer(serverStartTime) {
  /*
   * Never start a timer for a match that has already
   * been definitively finished.
   */
  if (state.matchFinished || !state.gameStarted) {
    console.warn("Ignoring timer start because match is not active.", {
      matchId: state.matchId,
  ...
  const startTime = Number(serverStartTime);

  if (!Number.isFinite(startTime) || startTime <= 0) {
    console.error("Invalid server start time:", serverStartTime);

    setStatus("Unable to start timer.");
  ...
  const update = () => {
    ...
    const remainingMs = state.challengeDeadline - Date.now();
...
function updateMatchTimer() {
  if (!elements.timerDiv || !state.gameStarted) {
    return;
  }
  ...
  elements.timerDiv.textContent = formatCountdown(remainingSeconds);
}
...
export function renderQuestions() {
  ...
  state.questions.forEach((q, questionIndex) => {
    ...
    if (q.passage) {
      ...
      passage.innerHTML = formatPassage(q.passage);
    ...
    elements.questionsDiv.appendChild(card);
  });
}
...
export function restoreSelectedAnswerUI() {
  ...
  state.selectedAnswers.forEach((choiceIndex, questionIndex) => {
    ...
    const card = elements.questionsDiv.children[questionIndex];
...
function lockAnswerButtons() {
  ...
  const choiceButtons = elements.questionsDiv.querySelectorAll(".choice");
...
export function selectAnswer(questionIndex, choiceIndex) {
  if (
    !state.gameStarted ||
    state.challengeSubmitted ||
    state.submissionInProgress ||
  ...
  state.selectedAnswers[questionIndex] = choiceIndex;
...
export function updateSubmitButton(forceEnabled = false) {
  if (!elements.submitButton) {
    return;
  }
...
export async function submitMatch(autoSubmitted = false) {
  /*
  ...
  if (
    ...
    console.error("Invalid selected answer state:", state.selectedAnswers);

    setStatus("Your answer state is invalid. Please reconnect to the match.");

    return;
  }

  if (!autoSubmitted) {
    const unanswered = state.selectedAnswers.filter(
      (answer) => answer === -1,
    ).length;
  ...
  const sent = sendRoomMessage({
    type: "submit_answers",
    answers: state.selectedAnswers.map((selected, questionIndex) => ({
      questionIndex,
      selected,
    })),
  });
  ...
  if (!sent) {
    ...
    setStatus("Connection to match was lost.");
...
export async function handleGameResult(data) {
  /* =========================================================
  ...
  const yourCorrect = Number(data.yourCorrect ?? 0);

  const yourTotal = Number(data.yourTotal ?? state.questions.length);

  const yourAccuracy =
    yourTotal > 0 ? Math.round((yourCorrect / yourTotal) * 100) : 0;

  let message;

  if (data.result === "win") {
    message = `You won! ${yourCorrect}/${yourTotal} (${yourAccuracy}%)`;
  } else if (data.result === "loss") {
    message = `You lost. ${yourCorrect}/${yourTotal} (${yourAccuracy}%)`;
  } else if (data.result === "tie") {
    message = `Tie! ${yourCorrect}/${yourTotal} (${yourAccuracy}%)`;
  } else {
    message = `Match complete: ${yourCorrect}/${yourTotal} (${yourAccuracy}%)`;
  }
  ...
  beginCooldown(data.cooldownUntil ?? data.nextGameAt ?? null);
  ...
  if (data.statsRecorded && data.statsRecorded.success === false) {
    console.error("SERVER FAILED TO RECORD STATS:", data.statsRecorded);

    setStatus("Match complete, but the server could not update your stats.");
  } else {
    const countdown = formatCountdown(
      Math.ceil(getCooldownRemainingMs() / 1000),
    );
...
export function handleSubmissionReceived(data = {}) {
  /*
...
function renderTopicPerformance(data) {
  ...
  const old = elements.resultDiv.querySelector(".current-topic-performance");

  if (old) {
    old.remove();
  }

  const questionResults = Array.isArray(data.questionResults)
  ...
  state.questions.forEach((question, questionIndex) => {
    ...
    const topic = normalizeTopic(
      question.topic || question.originalTopic || result.topic,
    );
  ...
  performance.className = "current-topic-performance";

  const heading = document.createElement("h3");

  heading.textContent = "This Match: Topic Performance";

  performance.appendChild(heading);
  ...
  topics.forEach(([topic, stats]) => {
    appendTopicRow(
      ...
      topic,
      stats,
      stats.total >= 2 && stats.correct / stats.total < 0.7,
    );
  });

  elements.resultDiv.appendChild(performance);
}
...
function renderResults(data) {
  ...
  state.questions.forEach((question, questionIndex) => {
    ...
    const selected = Number.isInteger(result.selected) ? result.selected : -1;

    const correctChoice = Number.isInteger(result.correctChoice)
      ? result.correctChoice
      : -1;

    const isCorrect =
      result.correct === true || result.correct === 1 || result.correct === "1";

    const oldBanner = card.querySelector(".question-result");
    ...
    resultBanner.className = `question-result ${
    ...
    if (isCorrect) {
      ...
      resultBanner.innerHTML = `
          ...
          <span>
            You selected:
            ${selected === -1 ? "No answer" : ["A", "B", "C", "D"][selected]}
          </span>

          <span>
            Correct answer:
            ${
              correctChoice >= 0 && correctChoice < 4
    ...
    card.insertBefore(resultBanner, card.firstChild);
    ...
    buttons.forEach((button, choiceIndex) => {
      ...
      if (choiceIndex === selected && selected !== correctChoice) {
...
function getCooldownDisplay() {
  let element = document.getElementById("cooldown-display");

  if (!element) {
    element = document.createElement("div");

    element.id = "cooldown-display";

    element.style.marginTop = "10px";

    element.style.fontWeight = "600";

    elements.startMatchButton?.parentElement?.appendChild(element);
  }
...
export function updateCooldownDisplay() {
  const element = getCooldownDisplay();
  ...
  if (remainingMs > 0) {
    element.textContent = `Next match available in ${formatCountdown(
      remainingMs / 1000,
    )}`;
    ...
    if (elements.startMatchButton) {
      ...
      elements.startMatchButton.textContent = `Cooldown: ${formatCountdown(
        remainingMs / 1000,
      )}`;
    }
  ...
  element.textContent = "You can join another match.";
...
if (elements.submitButton) {
  elements.submitButton.addEventListener("click", () => {
    ...
    setStatus("You can join the queue.");
  });
}

window.addEventListener("scoreladder:cooldown-started", () => {
  updateCooldownDisplay();
});

window.addEventListener("scoreladder:cooldown-tick", () => {
  updateCooldownUI();

  if (!state.gameStarted && state.cooldownUntil > Date.now()) {
    const countdown = formatCountdown(
      Math.ceil(getCooldownRemainingMs() / 1000),
    );

    setStatus(
      `Match complete. Cooldown: ${countdown} remaining. Analyze your previous game or practice your weakest topics on Khan Academy in the meantime.`,
    );
  }
});

window.addEventListener("scoreladder:cooldown-complete", () => {
  updateCooldownUI();
});
...
export { renderTopicPerformance, renderResults };
