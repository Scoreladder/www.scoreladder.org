...
export const RESUME_MATCH_STORAGE_KEY = "scoreladder_multiplayer_resume_match";

export const ACTIVE_MATCH_STATE_STORAGE_KEY =
  "scoreladder_multiplayer_active_match_state";

export const COOLDOWN_STORAGE_KEY = "scoreladder_multiplayer_cooldown_until";

export const TOTAL_QUESTIONS = 11;

export const MATCH_DURATION_MS = 13 * 60 * 1000;

export const COOLDOWN_DURATION_MS = 15 * 60 * 1000;

export const KHAN_ACADEMY_SAT_URL = "https://www.khanacademy.org/test-prep/sat";
...
export const TOPIC_DISPLAY_NAMES = {
  ...
  command_evidence_textual: "Command of Evidence — Textual",

  command_evidence_quantitative: "Command of Evidence — Quantitative",
...
export function getTopicDisplayName(topic) {
  ...
  return TOPIC_DISPLAY_NAMES[normalized] || topic || "Unknown Topic";
}
...
export function getStoredCooldownUntil() {
  try {
    ...
    console.error("Failed to read cooldown:", error);
...
export function startCooldownTimer() {
  ...
  state.cooldownInterval = setInterval(() => {
     ...
     */
    window.dispatchEvent(new CustomEvent("scoreladder:cooldown-tick"));

    if (getCooldownRemainingMs() <= 0) {
      stopCooldownTimer();

      window.dispatchEvent(new CustomEvent("scoreladder:cooldown-complete"));
    }
  }, 250);
}
...
export function beginCooldown(cooldownUntil = null) {
  const serverUntil = Number(cooldownUntil);

  const now = Date.now();

  const until =
    Number.isFinite(serverUntil) && serverUntil > now
      ? serverUntil
  ...
  window.dispatchEvent(new CustomEvent("scoreladder:cooldown-started"));
...
export function getStoredActiveMatchState() {
  try {
    const raw = localStorage.getItem(ACTIVE_MATCH_STATE_STORAGE_KEY);
    ...
    if (!parsed || typeof parsed !== "object") {
      localStorage.removeItem(ACTIVE_MATCH_STATE_STORAGE_KEY);

      return null;
    }

    if (!isLogicalMatchId(parsed.matchId)) {
      localStorage.removeItem(ACTIVE_MATCH_STATE_STORAGE_KEY);
    ...
    console.error("Failed to read active match state:", error);
...
export function isPersistedMatchExpired(saved) {
  ...
  const challengeDeadline = Number(saved.challengeDeadline) || 0;

  if (challengeDeadline > 0) {
    return challengeDeadline <= now;
  }

  const matchStartedAt = Number(saved.matchStartedAt) || 0;

  if (matchStartedAt > 0) {
    return matchStartedAt + MATCH_DURATION_MS <= now;
  }

  const savedAt = Number(saved.savedAt) || 0;

  if (savedAt > 0) {
    return savedAt + MATCH_DURATION_MS <= now;
  }
...
export function saveActiveMatchState() {
  if (!isLogicalMatchId(state.matchId)) {
    console.warn("Refusing to persist invalid logical matchId:", state.matchId);

    return false;
  }

  if (
    !Number.isFinite(Number(state.matchStartedAt)) ||
  ...
  const activeMatchState = {
    ...
    selectedAnswers: Array.isArray(state.selectedAnswers)
      ? [...state.selectedAnswers]
      : [],

    challengeDeadline: Number(state.challengeDeadline) || 0,

    timeRemaining: Number(state.timeRemaining) || 0,

    gameStarted: state.gameStarted === true,

    playerReady: state.playerReady === true,

    matchConnectionConfirmed: state.matchConnectionConfirmed === true,

    challengeSubmitted: state.challengeSubmitted === true,

    answerSelectionLocked: state.answerSelectionLocked === true,

    matchFinished: state.matchFinished === true,

    matchStartedAt: Number(state.matchStartedAt) || 0,
  ...
  try {
    localStorage.setItem(
      ...
      JSON.stringify(activeMatchState),
    );

    localStorage.setItem(RESUME_MATCH_STORAGE_KEY, state.matchId);
    ...
    console.error("Failed to save active match state:", error);
...
export function clearActiveMatchState() {
  try {
    ...
    console.error("Failed to clear active match state:", error);
  }

  clearResumeMatch();
}

export function updatePersistedSelectedAnswers(selectedAnswers) {
...
export function saveResumeMatch(matchId) {
  ...
  if (!isLogicalMatchId(normalizedMatchId)) {
    console.warn("Refusing to save invalid resume match ID:", matchId);
  ...
  state.resumeMatchId = normalizedMatchId;

  try {
    localStorage.setItem(RESUME_MATCH_STORAGE_KEY, normalizedMatchId);
  } catch (error) {
    console.error("Failed to save resume match:", error);
  }
}

export function getStoredResumeMatchId() {
  try {
    const matchId = localStorage.getItem(RESUME_MATCH_STORAGE_KEY);
    ...
    if (!isLogicalMatchId(normalizedMatchId)) {
      ...
      localStorage.removeItem(RESUME_MATCH_STORAGE_KEY);
    ...
    console.error("Failed to read resume match:", error);
...
export function clearResumeMatch() {
  ...
  try {
    ...
    console.error("Failed to clear resume match:", error);
  }
}
...
export function isResumeAvailable() {
  ...
  if (activeState && isLogicalMatchId(activeState.matchId)) {
    ...
    state.resumeMatchId = activeState.matchId;
  ...
  return (
    state.resumeAvailable === true && isLogicalMatchId(state.resumeMatchId)
  );
}
...
export function restoreActiveMatchState() {
  ...
  const logicalMatchId = isLogicalMatchId(saved?.matchId)
    ? saved.matchId.trim()
    : null;
  ...
  state.matchId = logicalMatchId;

  state.playerId = saved?.playerId ?? state.playerId ?? null;

  state.opponent = saved?.opponent ?? null;

  state.questions = Array.isArray(saved?.questions) ? saved.questions : [];

  state.selectedAnswers = Array.isArray(saved?.selectedAnswers)
    ? [...saved.selectedAnswers]
    : [];

  state.challengeDeadline = Number(saved?.challengeDeadline) || 0;

  state.timeRemaining = Number(saved?.timeRemaining) || 0;

  state.matchStartedAt = Number(saved?.matchStartedAt) || 0;
  ...
  state.playerReady = saved?.playerReady === true;
  ...
  state.resumeMatchId = logicalMatchId;
  ...
  state.answerSelectionLocked = saved?.answerSelectionLocked === true;

  /* -------------------------------------------------------
     REPAIR LOGICAL MATCH ID
     ------------------------------------------------------- */

  if (saved && saved.matchId !== logicalMatchId) {
    try {
      const repairedState = {
        ...saved,
      ...
      console.error("Failed to repair active match state:", error);
    }
  }
  ...
  try {
    localStorage.setItem(RESUME_MATCH_STORAGE_KEY, logicalMatchId);
  } catch (error) {
