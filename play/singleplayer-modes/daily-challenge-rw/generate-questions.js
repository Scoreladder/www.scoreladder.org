...
const QUESTION_API_URL = "https://dailyai-rw.scoreladder.org/";

// Auth API: handles sessions, users, stats, and score submission
const AUTH_API_URL = "https://auth.scoreladder.org";
...
async function getCurrentUser() {
  try {
    ...
    if (!sessionId) {
      try {
        sessionId = sessionStorage.getItem(LOCAL_SESSION_KEY);
      } catch (error) {
        console.error("Failed to read session:", error);
      }
    }

    let meUrl = `${AUTH_API_URL}/me`;
...
async function loadDailyQuestions() {
  ...
  try {
    ...
    try {
      ...
      console.error("Question worker returned invalid JSON:", responseText);

      throw new Error("The question worker returned invalid JSON.", {
        cause: parseError,
      });
    }
    ...
    if (!res.ok) {
      throw new Error(
        data?.error || `Question worker returned HTTP ${res.status}`,
      );
    }
    ...
    if (rawQuestions.length === 0) {
      throw new Error("No questions were returned by the question worker.");
    }
    ...
    const candidateQuestions = rawQuestions.slice(0, 10);

    const invalidQuestions = [];

    const hasValidQuestion = (question, index) => {
      /*
      ...
      if (!normalizedTopic) {
        problems.push(`invalid topic "${question?.topic ?? "(missing)"}"`);
      }
      ...
      if (
        ...
        problems.push(`invalid answer "${question?.answer}"`);
      }
      ...
      if (
        ...
        problems.push("missing question text");
      }

      if (typeof question?.passage !== "string") {
        problems.push("missing passage");
      }
    ...
    const allValid = candidateQuestions.every(hasValidQuestion);

    /*
     * Require exactly 10 questions.
     */

    if (candidateQuestions.length !== 10 || !allValid) {
      console.error("Invalid daily questions:", {
        received: rawQuestions.length,
        accepted: candidateQuestions.length,
        invalidQuestions,
      });

      if (candidateQuestions.length !== 10) {
        throw new Error(
      ...
      if (invalidQuestions.length > 0) {
        const details = invalidQuestions
          .map((item) => `Question ${item.index}: ${item.problems.join(", ")}`)
          .join("; ");

        throw new Error(`Invalid daily question data. ${details}`);
      }

      throw new Error("The daily challenge contained invalid questions.");
    }
    ...
    questionsDiv.innerHTML = `
            <div class="card">
                ...
                <p>
                    ${escapeHtml(
                      err?.message || "Unknown question-loading error.",
                    )}
                </p>
            </div>
        `;
...
async function startChallenge() {
  ...
  if (!user) {
    alert("You must be logged in to complete the daily challenge.");
  ...
  challengeDeadline = Date.now() + 660000;
...
function renderQuestions() {
  ...
  questions.forEach((q, questionIndex) => {
    ...
    shuffled.forEach((choice, choiceIndex) => {
...
function selectAnswer(questionIndex, choiceIndex) {
  ...
  const card = questionsDiv.children[questionIndex];
...
function updateSubmitButton(forceEnabled = false) {
...
async function submitChallenge(autoSubmitted = false) {
  ...
  if (!autoSubmitted) {
    const unanswered = selectedAnswers.filter((answer) => answer === -1).length;
  ...
  try {
    ...
    questions.forEach((q, questionIndex) => {
      ...
      if (selectedChoice !== -1) {
        const shuffledChoice = q._shuffledChoices[selectedChoice];

        isCorrect = shuffledChoice.originalIndex === q.answer;
      ...
      if (!normalizedTopic) {
        console.error("Cannot submit unknown topic:", q.topic);

        throw new Error(`Unknown question topic: ${q.topic}`);
      }
    ...
    try {
      ...
      sessionId =
        params.get("session") || sessionStorage.getItem(LOCAL_SESSION_KEY);
    } catch (error) {
      console.error("Failed to retrieve session:", error);
    }

    let saveUrl = `${AUTH_API_URL}/daily-complete`;

    if (sessionId) {
      saveUrl = `${AUTH_API_URL}/daily-complete?session=${encodeURIComponent(
        sessionId,
      )}`;
    }
    ...
    if (saveResponse.status === 409) {
      throw new Error(
        saveData?.error || "You have already completed today's challenge.",
      );
    }
    ...
    autoSubmitFailed = autoSubmitted;

    updateSubmitButton(autoSubmitted);
    ...
    if (
      ...
      challengeDeadline = Date.now() + timeRemaining * 1000;
...
function renderResults(correct, total, accuracy, autoSubmitted) {
  resultDiv.innerHTML = `
  ...
  questions.forEach((q, questionIndex) => {
    ...
    const correctChoice = q._shuffledChoices.findIndex(
      (choice) => choice.originalIndex === q.answer,
    );

    const buttons = card.querySelectorAll(".choice");

    const isCorrect = selected !== -1 && selected === correctChoice;
...
function formatPassage(text) {
  ...
  return escapeHtml(text)
    .replace(/\[UNDERLINED\]\((.*?)\[\/UNDERLINED\]\)/g, "<u>$1</u>")
    .replace(/\n/g, "<br>");
}
...
submitButton.addEventListener("click", () => submitChallenge(autoSubmitFailed));
